...
export default function MyCarsScreen() {
  ...
  // Hooks
  const { data: vehiclesData, isLoading, refetch } = useGetVehiclesQuery(undefined, {
    skip: isAdmin,
  });
  ...
  const renderCar = ({ item }: { item: Car }) => {
    ...
    const isSubscribed = isVehicleSubscribed(item.id);
  ...
  return (
    <TouchableWithoutFeedback onPress={handleDismiss} accessible={false}>
      <View style={styles.container}>
        ...
        {/* TYPE PICKER MODAL - Kept for fallback but unused since we used ScrollView above */}
