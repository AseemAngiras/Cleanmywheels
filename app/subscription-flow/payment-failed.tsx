...
export default function SubscriptionPaymentFailedScreen() {
  const router = useRouter();
  const params = useLocalSearchParams();
  const insets = useSafeAreaInsets();
  const { grandTotal, serviceName } = params;
  ...
  return (
    <ScreenWrapper backgroundColor={Colors.background}>
      ...
      >
        ...
        <View className="bg-card rounded-b-[44px] items-center pt-16 pb-12 px-6 shadow-xl border-b border-border/30">
          ...
          <Text className="text-[14px] color-textSecondary font-[600] mt-2 text-center px-4">
            We couldn't process your subscription payment for {serviceName || "Premium Wash"}. Please try again.
          </Text>
        </View>

        {/* Possible Reasons */}
        <View className="px-6 mt-10">
          <Text className="text-[11px] font-[800] color-textSecondary uppercase tracking-[2.5px] mb-6 px-1">
            Common reasons
          </Text>

          <View className="gap-y-4">
            <View className="flex-row items-center bg-card p-5 rounded-[24px] border border-border/50">
      ...
      {/* Footer Buttons */}
      <View
        className="absolute bottom-0 left-0 right-0 p-6 bg-background border-t border-border/30 gap-y-3"
